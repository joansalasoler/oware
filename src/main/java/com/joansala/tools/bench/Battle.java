...
public final class Battle {
    ...
    private static Roots[] roots = null;

    /** Endgames used for each engine */
    private static Leaves[] leaves = null;
    ...
    private static void printSettings() {
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            if (roots[i] instanceof Book) {
                bookName = ((Book) roots[i]).getField("Name");
            }

            if (leaves[i] instanceof Book) {
                leavesName = ((Book) leaves[i]).getField("Name");
            }
